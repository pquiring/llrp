...
/**
 * This is the response by the Reader to a GET_ACCESSSPECS command. If there are no AccessSpecs configured at the Reader, the response is just the LLRPStatus parameter with the
 * success code. Else, a list of (AccessSpecID, AccessSpec parameter) is returned by the Reader, along with the LLRPStatus parameter containing the success code. The order of the
 * AccessSpecs listed in the message is normatively the order in which the AccessSpecs were created at the Reader.
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=63&view=fit">LLRP Specification Section 11.1.10</a>} and {@link
...
public class GET_ACCESSSPECS_RESPONSE extends LLRPMessage {
  ...
  private static final Logger LOGGER = LogManager.getLogger(GET_ACCESSSPECS_RESPONSE.class);
