...
/**
 * This message is issued by the Client to the Reader. Upon receiving the message, the Reader moves the AccessSpec corresponding to the AccessSpecID in this message from the
 * Disabled state to the Active state. The Reader executes this access-spec until it gets a DISABLE_ACCESSSPEC or a DELETE_ACCESSSPEC from the Client. The AccessSpec takes effect
 * with the next (and subsequent) inventory rounds.AccessSpecID: If set to 0, all AccessSpecs are enabled.
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=62&view=fit">LLRP Specification Section 11.1.5</a>} and {@link
...
public class ENABLE_ACCESSSPEC extends LLRPMessage {
  ...
  private static final Logger LOGGER = LogManager.getLogger(ENABLE_ACCESSSPEC.class);
