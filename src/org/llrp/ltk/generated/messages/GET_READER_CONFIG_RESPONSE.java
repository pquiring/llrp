...
/**
 * This is the response by the Reader to the GET_READER_CONFIG message. The response is the LLRPStatus Parameter and the list of configuration parameters based on the RequestedData
 * in GET_READER_CONFIG. If the GET_READER_CONFIG message did not have any errors, the success code is returned in the LLRPStatus parameter, and in addition the requested
 * configuration parameters are returned. If there is an error, the appropriate error code is returned in the LLRPStatus parameter. The response contains at most one instance of
 * each configuration parameter except for two cases, which are as follows:If RequestedData is 0, 2 or 3, and AntennaID is set to 0 in the GET_READER_CONFIG message, the Reader
 * SHALL return one instance of AntennaProperties Parameter or AntennaConfiguration Parameter per requested antenna.If RequestedData is 0 or 9 (10), and GPIPortNum (GPOPortNum) is
 * set to 0 in the GET_READER_CONFIG message, and, if the Reader supports GPI (GPO), the Reader SHALL return one instance of GPIPortCurrentState (GPOWriteData) Parameter per
 * requested GPI Port (GPO Port).
 *
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=68&view=fit">LLRP Specification Section 12.1.2</a>} and {@link
...
public class GET_READER_CONFIG_RESPONSE extends LLRPMessage {
  ...
  private static final Logger LOGGER = LogManager.getLogger(GET_READER_CONFIG_RESPONSE.class);
