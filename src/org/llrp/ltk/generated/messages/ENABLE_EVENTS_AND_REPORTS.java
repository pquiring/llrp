...
/**
 * This message can be issued by the Client to the Reader after a LLRP connection is established. The Client uses this message to inform the Reader that it can remove its hold on
 * event and report messages. Readers that are configured to hold events and reports on reconnection (See Section 12.2.6.4) respond to this message by returning the tag reports
 * accumulated (same way they respond to GET_REPORT (See Section 13.1.1)).
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=78&view=fit">LLRP Specification Section 13.1.6</a>} and {@link
...
public class ENABLE_EVENTS_AND_REPORTS extends LLRPMessage {
  ...
  private static final Logger LOGGER = LogManager.getLogger(ENABLE_EVENTS_AND_REPORTS.class);
