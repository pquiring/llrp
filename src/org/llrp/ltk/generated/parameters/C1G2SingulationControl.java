...
/**
 * This C1G2SingulationControl Parameter provides controls particular to the singulation process in the C1G2 air protocol. The singulation process is started using a Query command
 * in the C1G2 protocol. The Query command describes the session number, tag state, the start Q value to use, and the RF link parameters. The RF link parameters are specified using
 * the C1G2RFControl Parameter (see section 15.2.1.2.1.2). This Singulation Parameter specifies the session, tag state and description of the target singulation environment. The
 * following attributes are specified to provide guidance to the Reader for the singulation algorithm:Tag transit time: This is the measure of expected tag mobility in the field of
 * view of the antenna where this inventory operation is getting executed.Tag population: This is the expected tag population in the field of view of the antenna. In addition, the
 * Singulation Parameter allows setting of the following:Session ID: This is the C1G2 session number that the tags use to update the inventory state upon successful
 * singulation.TagInventoryStateAwareSingulationAction: This is used if the TagInventoryStateAware flag is set to true in the InventoryParameterSpec. I: This is the inventoried
 * state of the target tag population in the selected session. Only tags that match the session state participate in the inventory round. If the Ignore value is specified, the
 * Reader ignores this field, and its up to the Reader implementation to determine the value of I used in the inventory round. S: This is the state of the SL flag in the tag. Only
 * tags that match that tag state participate in the inventory round. If the Ignore value is specified, the Reader ignores this field, and its up to the Reader implementation to
 * determine the value of S used in the inventory round.
 *
 ...
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=104&view=fit">LLRP Specification Section 15.2.1.2.1.3</a>} and {@link
...
public class C1G2SingulationControl extends TLVParameter {
  ...
  private static final Logger LOGGER = LogManager.getLogger(C1G2SingulationControl.class);
