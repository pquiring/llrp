...
/**
 * This parameter describes the contents of the event notification sent by the Reader, and defines the events that cause the notification to be sent. Event notification messages
 * may be sent by the Reader due to connection establishment/closing event, critical events such as hopping, fault-detection in a Reader functional block, buffer overflow, due to
 * the activation of a Reader accessory trigger input (e.g. motion detection), or due to performance monitoring events such as abnormalities in the RF environment.Timestamp is the
 * time that the events reported occurred.LLRP assumes a reliable stream transport mechanism. Messages sent through LLRP will arrive in the order that they were sent over the
 * transport and binding utilized. Status events within the same message SHALL be ordered chronologically.Status events delivered by reader event notifications are useful,
 * especially in conjunction with the tag report data. The following describes the requirements of the reader event notifications ordering with respect to the ordering of tag
 * reports and Reader Event Notifications. The following requirements are made on the ordering of Event Parameters with respect to each other and to tag report Parameters. These
 * statements apply if the respective status events and report triggers are enabled. If the start of an ROSpec is triggered by a GPI, the GPIEvent Parameter SHALL be sent before
 * the ROSpecEvent Parameter signaling the start of the ROSpec.If the end of an ROSpec is triggered by a GPI, the GPIEvent Parameter SHALL be sent before the ROSpecEvent Parameter
 * signaling the end of the ROSpec.If an ROSpec contains one or more AISpecs, the ROSpecEvent parameter signaling the end of an ROSpec SHALL be sent after the AISpecEvent Parameter
 * signaling the end of the last AISpec within that ROSpec.If one ROSpec pre-empts another ROSpec, the ROSpecEvent parameter signaling the preemption of the first ROSpec SHALL be
 * sent before the ROSpecEvent parameter signaling the start of the next ROSpec.Tag data received during an ROSpec execution SHALL be sent between the ROSpecEvent parameter
 * signaling the start of the ROSpec and the ROSpecEvent parameter signaling the end or preemption of the ROSpec if the ROReportTrigger is not set to "None".Tag data received
 * during an AISpec execution SHALL be sent before the AISpecEvent Parameter signaling the end of the AISpec if the ROReportTrigger is not "None" or "end of RO Spec"Tag data
 * received during the time on a channel SHALL be sent after the HoppingEvent parameter that announced this channel and before the next HoppingEvent parameter when the
 * ROReportTrigger is not "None" and N=1.
 *
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=86&view=fit">LLRP Specification Section 13.2.6</a>} and {@link
...
public class ReaderEventNotificationData extends TLVParameter {
  ...
  private static final Logger LOGGER = LogManager.getLogger(ReaderEventNotificationData.class);
