...
/**
 * MB is the memory bank to use. WordPtr is the starting word address. Write Data is the data to be written to the tag. Word Count is the number of words to be written. Depending
 * on the word count, the Reader may have to execute multiple C1G2 air protocol Write commands. Access Password is the password used by the Reader to transition the tag to the
 * secure state so that it can write to protected tag memory regions.
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=107&view=fit">LLRP Specification Section 15.2.1.3.2.2</a>} and {@link
...
public class C1G2Write extends TLVParameter implements AccessCommandOpSpec {
  ...
  private static final Logger LOGGER = LogManager.getLogger(C1G2Write.class);
