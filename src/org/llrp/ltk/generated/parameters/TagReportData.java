...
/**
 * This report parameter is generated per tag per accumulation scope. The only mandatory portion of this parameter is the EPCData parameter. If there was an access operation
 * performed on the tag, the results of the OpSpecs are mandatory in the report. The other sub-parameters in this report are optional. LLRP provides three ways to make the tag
 * reporting efficient:Allow parameters to be enabled or disabled via TagReportContentSelector (section 13.2.1.1) in TagReportSpec. If an optional parameter is enabled, and is
 * absent in the report, the Client SHALL assume that the value is identical to the last parameter of the same type received. For example, this allows the Readers to not send a
 * parameter in the report whose value has not changed since the last time it was sent by the Reader.Allow accumulation of tag reports. See next section for details of
 * accumulation. A Reader MAY accumulate multiple tag reports into a single tag report.. If a Reader accumulates, the Reader SHALL follow the accumulation rules specified in this
 * section. The following specifies the rules for accumulating multiple tag observations into a single TagReportData:EPCData:The Reader SHALL not accumulate tag reports that do not
 * have the same EPCData value.OpSpecResultList:The Reader SHALL not accumulate tag reports that do not have the same value for the OpSpec results in the OpSpecResultList.SpecID,
 * SpecIndex, InventoryParameterSpecID, AntennaID, AirProtocolTagData, AccessSpecID:These fields are optional, and their reporting can be enabled by the Client. If the Client has
 * enabled one or more fields listed above, the Reader SHALL not accumulate tag reports that do not have the same value for all the enabled fields.FirstSeenTimestamp,
 * LastSeenTimestamp, PeakRSSI, TagSeenCount, ChannelIndexThese fields are optional, and their reporting can be enabled by the Client. If the field is enabled, the Reader sets the
 * value of these fields as follows:FirstSeenTimestamp: The Reader SHALL set it to the time of the first observation amongst the tag reports that get accumulated in the
 * TagReportData.LastSeenTimestamp: The Reader SHALL set it to the time of the last observation amongst the tag reports that get accumulated in the TagReportData.PeakRSSI: The
 * Reader SHALL set it to the maximum RSSI value observed amongst the tag reports that get accumulated in the TagReportData.ChannelIndex: The Reader MAY set it to the index of the
 * first channel the tag was seen.TagSeenCount: The Reader SHALL set it to the number of tag reports that get accumulated in the TagReportData.
 *
 *
 *
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=80&view=fit">LLRP Specification Section 13.2.3</a>} and {@link
...
public class TagReportData extends TLVParameter {
  ...
  private static final Logger LOGGER = LogManager.getLogger(TagReportData.class);
