...
/**
 * For a particular antenna, this parameter specifies the Reader's valid index range in the Receive Sensitivity Table. A Reader should report this parameter if the Reader allows
 * control of receive sensitivity (i.e., the Reader reports a Receive Sensitivity Table with more than one entry) and the Reader supports multiple antennas where the antennas can
 * have different receive sensitivity values.If this parameter is omitted, then the Client SHALL assume that for all of the Reader's antennas the index range is the same as in the
 * Receive Sensitivity Table.
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=46&view=fit">LLRP Specification Section 9.2.1.2</a>} and {@link
...
public class PerAntennaReceiveSensitivityRange extends TLVParameter {
  ...
  private static final Logger LOGGER = LogManager.getLogger(PerAntennaReceiveSensitivityRange.class);
