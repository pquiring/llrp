...
/**
 * This parameter contains the definition of the access privilege updates (read/write/permalock) to be performed in various locations of the memory. The five data fields for which
 * we can define access control using the lock command are: Kill Password, Access Password, EPC memory, TID memory and User memory. The access privilege updates are expressed as a
 * list of C1G2LockPayload Parameters, one for each memory location.The Access Password provides the password to enter the secured state. A Reader can perform a lock operation on a
 * tag only if the tag is in the secured state. The tag enters the secured state only using the Access Password (if a non-zero value).
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=108&view=fit">LLRP Specification Section 15.2.1.3.2.4</a>} and {@link
...
public class C1G2Lock extends TLVParameter implements AccessCommandOpSpec {
  ...
  private static final Logger LOGGER = LogManager.getLogger(C1G2Lock.class);
