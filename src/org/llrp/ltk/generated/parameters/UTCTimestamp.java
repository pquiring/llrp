...
/**
 * The timestamps in LLRP messages or parameters can be either the uptime or the UTC time [UTC]. If a Reader has an UTC clock, all timestamps reported by the Reader SHALL use an
 * UTC timestamp parameter. If a Reader has no UTC clock capability, all timestamps reported by the Reader. SHALL use the uptime parameter.
 *
 * See also {@link <a href="http://www.epcglobalinc.org/standards/llrp/llrp_1_0_1-standard-20070813.pdf#page=37&view=fit">LLRP Specification Section 7.1.3.1.1.1</a>} and {@link
...
public class UTCTimestamp extends TLVParameter implements Timestamp {
  ...
  private static final Logger LOGGER = LogManager.getLogger(UTCTimestamp.class);
